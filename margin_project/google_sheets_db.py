...
def save_auth_state(spreadsheet_id, session_id, auth_state):
    """Сохраняет состояние авторизации в Google Sheets (лист "AuthState")."""
    sheet = get_or_create_spreadsheet(spreadsheet_id)
    auth_worksheet = sheet.worksheet("AuthState")
    
    # Получаем все текущие записи
    all_auth = auth_worksheet.get_all_values()
    print(f"Текущие данные в AuthState: {all_auth}")
    
    # Проверяем, существует ли запись для этого session_id
    session_exists = False
    for i, row in enumerate(all_auth[1:], start=1):  # Начиная со второй строки (пропускаем заголовок)
        if row[0] == session_id:
            # Обновляем существующую запись
            auth_worksheet.update(f"A{i + 1}:C{i + 1}", [[session_id, str(auth_state['authenticated']).upper(), auth_state['user']]])
            session_exists = True
            print(f"Обновлена запись для session_id: {session_id}")
            break
    
    if not session_exists:
        # Добавляем новую запись
        auth_worksheet.append_row([session_id, str(auth_state['authenticated']).upper(), auth_state['user']])
        print(f"Добавлена новая запись для session_id: {session_id}")

def load_auth_state(spreadsheet_id, session_id):
    """Загружает состояние авторизации из Google Sheets (лист "AuthState")."""
    sheet = get_or_create_spreadsheet(spreadsheet_id)
    auth_worksheet = sheet.worksheet("AuthState")
    all_auth = auth_worksheet.get_all_values()
    
    # Проверяем заголовки
    if not all_auth or all_auth[0][0] != "SessionID":
        print(f"Ошибка: Неправильный заголовок в листе AuthState. Текущие заголовки: {all_auth[0] if all_auth else 'Пусто'}")
        return {'authenticated': False, 'user': ''}
    
    # Ищем запись по session_id
    for row in all_auth[1:]:  # Пропускаем заголовок
        if row[0] == session_id:
            return {
                'authenticated': row[1].strip().upper() == "TRUE",
                'user': row[2] if row[2] else ''
            }
    
    print(f"Запись для session_id {session_id} не найдена, возвращаем состояние по умолчанию")
    return {'authenticated': False, 'user': ''}
