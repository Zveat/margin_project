...
credentials = {
    "usernames": {
        "zveat": {
            "name": "zveat",
            "password": bcrypt.hash("aydia6c4")  # Хэшированный пароль
        },
        "jane": {
            "name": "Jane Doe",
        ...
        for index, product in enumerate(st.session_state.products):
            ...
            with st.expander(f"🛒 {product['Товар']} ({product['Количество']} {product['Ед_измерения']})"):
                ...
                with col2:
                    ...
                    product_weight = product.get("Вес (кг)", 0)  # Защита от отсутствия ключа
                    st.write(f"**Вес:** {product_weight if product_weight > 0 else 'Не указан'} кг")
