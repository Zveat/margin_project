...
def run_margin_service():
    ...
    st.markdown(
        ...
        /* Добавляем большой отступ под списком товаров */
        .stExpander > div[data-testid="stVerticalBlock"] {
            margin-bottom: 80px; /* Сохраняем большой отступ под списком товаров */
        }
        /* Добавляем отступ между текстом и кнопками внутри экспандера */
        .stExpander div[data-testid="stVerticalBlock"] div:last-child {
            margin-top: 20px; /* Отступ перед кнопками внутри экспандера */
        }
        ...
        for index, product in enumerate(st.session_state.products):
            ...
            with st.expander(f"🛒 {product['Товар']} ({product['Количество']} {product['Ед_измерения']})"):
                ...
                with col1:
                    ...
                    st.write(" ")  # Добавляем пустую строку для отступа перед кнопками
                with col2:
