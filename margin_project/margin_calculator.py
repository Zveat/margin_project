...
def run_margin_service():
    # CSS для единообразия в «Калькуляторе маржинальности» с оптимизированными отступами
    st.markdown(
        """
        <style>
        ...
        /* Оптимизируем отступ под списком товаров */
        .stExpander > div[data-testid="stVerticalBlock"] {
            margin-bottom: 40px; /* Уменьшаем отступ под списком товаров для более компактного вида */
        }
        /* Добавляем небольшой отступ между текстом и кнопками внутри экспандера */
        .stExpander div[data-testid="stVerticalBlock"] div:last-child {
            margin-top: 15px; /* Уменьшаем отступ перед кнопками до 15px для аккуратности */
        }
        ...
        for index, product in enumerate(st.session_state.products):
            ...
            with st.expander(f"🛒 {product['Товар']} ({product['Количество']} {product['Ед_измерения']})"):
                ...
                with col1:
                    ...
                    st.write(" ")  # Добавляем одну пустую строку для минимального, но заметного отступа перед кнопками
                with col2:
    ...
    # Сохраняем отступ после списка товаров и перед расчётами
