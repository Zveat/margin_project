...
def run_margin_service():
    ...
    st.markdown(
        ...
        .stExpander > div[data-testid="stVerticalBlock"] {
            margin-bottom: 80px; /* Увеличенный отступ под каждым экспандером для заметного пространства */
        }
    ...
    # Уберём ненужный отступ перед "Итоговыми расчётами" и добавим отступ после списка товаров
    st.write(" ")  # Пустая строка для небольшого отступа после списка товаров
