...
def run_margin_service():
    # CSS для единообразия в «Калькуляторе маржинальности» с увеличенным отступом
    st.markdown(
        """
        <style>
        ...
        .stExpander > div[data-testid="stVerticalBlock"] {
            margin-bottom: 60px; /* Увеличенный отступ под каждым экспандером для большего пространства */
        }
        /* Убираем лишние отступы для кнопок внутри экспандера, если нужно */
    ...
    if not st.session_state.products:
        ...
        for index, product in enumerate(st.session_state.products):
            ...
            with st.expander(f"🛒 {product['Товар']} ({product['Количество']} {product['Ед_измерения']})"):
                ...
                col_btn_edit, col_btn_delete = st.columns([4, 1])  # Увеличиваем пропорцию для левой колонки, чтобы "Удалить" был прижат к правому краю
                with col_btn_edit:
    ...
    # Добавляем явный и заметный отступ перед блоком расчётов
    st.write(" ")  # Пустая строка для дополнительного пространства
    st.write(" ")  # Ещё одна пустая строка
    st.markdown("### Итоговые расчёты")  # Заголовок для разделения
